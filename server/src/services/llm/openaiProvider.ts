    ...
    try {
      const response = await this.client.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'Você é um especialista em revisão de documentos legais. Forneça sugestões práticas e específicas.',
          },
          {
            role: 'user',
            content: prompt,
    ...
    try {
      const response = await this.client.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'Você é um especialista em conformidade legal. Seja preciso e objetivo.',
          },
          {
            role: 'user',
            content: prompt,
